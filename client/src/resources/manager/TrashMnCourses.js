...
function TrashMnCourses() {
  ...
  // function
  document.title = "Khóa học đã xóa "
  ...
  const deleteCourse = (deleteID) => (e) => {
    ...
    axios({
      ...
      data: {},
      withCredentials: true,
      url: `${REACT_APP_SERVER}/courses/${deleteID}/force`
    })
    .then(ketqua => {
  ...
  const restoreOne = (courseID) => (e) => {
    ...
    axios({
      ...
      data: {},
      withCredentials: true,
      url: `${REACT_APP_SERVER}/courses/${courseID}/restore`
    })
    .then(ketqua => {
      // console.log(typeof(ketqua.data), ketqua.data);
      if(!ketqua.data) {
        ...
        <form>
            <div className="container-xl">
              <div className="row mt-3">
                  <h3 className="mb-0 col-sm-8"><b>Khóa học đã xóa</b></h3>
                  <div className="col-sm-4 text-right">
                      <Link className="btn btn-danger btn-sm ml-auto mr-2" title="Khóa học đã xóa" to={"/manager/Courses"}><i className="fas fa-arrow-left"></i> Quay lại</Link>
                  </div>
              </div>
                    ...
                    {course.toString() ? course.map((course,index)=>(
                      <tbody key={index}>
                          <tr>
                              ...
                              <td>
                                <Link to={'#'} className="btn btn-link btn-restore" title="Khôi phục" onClick={restoreOne(course._id)}>Khôi phục</Link>
                                <Link to="#" className="btn btn-link" title="Xóa" data-toggle="modal" data-target="#delete-course-model" title="Xóa vĩnh viễn" onClick={()=>{setDeleteID(course._id)}}>Xóa vĩnh viễn</Link>
                              </td>
                          </tr>
                      </tbody>
                    ))
        ...
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <p>Bạn có chắc muốn xóa khóa học này, hành động này sẽ không thể khôi phục?</p>
            </div>
